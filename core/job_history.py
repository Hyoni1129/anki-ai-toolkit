...
class JobHistoryManager:
    ...
    def append_items(self, job_id: str, items: List[Dict[str, Any]]) -> None:
        ...
        if not job_data:
            logger.warning("Cannot append items; job %s not found", job_id)
            return
    ...
    def reinsert_job(self, job_id: str, overwrite: bool = True) -> Dict[str, int]:
        ...
        for item in items:
            try:
                ...
                if asset_path:
                    ...
                    note[target_field] = str(item.get("api_output", ""))

                secondary_field = str(item.get("secondary_field", "")).strip()
                secondary_output = str(item.get("secondary_output", ""))
                if secondary_field and secondary_field in note and secondary_output:
                    note[secondary_field] = secondary_output

                mw.col.update_note(note)
                success += 1

            except Exception as exc:
                logger.error("Reinsert failed for job %s item %s: %s", job_id, item, exc)
                failed += 1

        try:
            mw.col.save()
        except Exception as exc:
            logger.warning("Collection save after reinsertion failed: %s", exc)
